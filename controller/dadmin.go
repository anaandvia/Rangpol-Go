...
func DataAdminController(c *fiber.Ctx) error {
	...
	if err := database.DBConn.Where("dlt = ? and level = ?", 0, 2).Find(&users).Error; err != nil {
...
func CreateAdminController(c *fiber.Ctx) error {
	sess, err := middleware.GetSessionStore().Get(c)
	if err != nil {
	...
	if err := database.DBConn.Where("nim = ? AND dlt = ? ", nim, 0).First(&existingUser).Error; err == nil {
		...
		return c.Redirect("/admin/dataadmin")
	}
	...
	sess.Set("flash_success", "Admin created successfully")
...
func DeleteAdminController(c *fiber.Ctx) error {
	sess, err := middleware.GetSessionStore().Get(c)
	if err != nil {
		log.Println("Error getting session:", err)
		return err
...
func UpdateAdminController(c *fiber.Ctx) error {
	sess, err := middleware.GetSessionStore().Get(c)
	if err != nil {
		log.Println("Error getting session:", err)
		return err
